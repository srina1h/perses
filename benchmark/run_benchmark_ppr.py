...
INSTALLS = {
    "ppr": os.path.join(__location__, "binaries", "update_ppr.sh")
}

REDUCERS = {
    "ppr": os.path.join(__location__, "binaries", "run_ppr.sh")
}
...
def extract_info_properties(subject_name: str) -> Tuple[str, str]:
    """Extract script file, sed file and variant file from a subject folder"""
    info_dict = dict()
    ...
    # validate seed file, variant file and script file path
    if "seed_file" not in info_dict:
        raise Exception('Error: No seed_file found in info.properties')
    if "variant_file" not in info_dict:
        raise Exception('Error: No variant_file found in info.properties')
    if "script_file" not in info_dict:
        raise Exception('Error: No script_file found in info.properties')

    seed_file_path = os.path.join(__location__, subject_name, info_dict["seed_file"])
    variant_file_path = os.path.join(__location__, subject_name, info_dict["variant_file"])
    script_file_path = os.path.join(__location__, subject_name, info_dict["script_file"])

    if not os.path.exists(seed_file_path):
        raise Exception('Error: seed_file not found: {}'.format(seed_file_path))
    if not os.path.exists(variant_file_path):
        raise Exception('Error: variant_file not found: {}'.format(variant_file_path))
    if not os.path.exists(script_file_path):
        raise Exception('Error: script_file not found: {}'.format(script_file_path))

    return seed_file_path, variant_file_path, script_file_path


def count_token(seed_file_path: str) -> int:
    try:
        token_counter_sh = os.path.join(__location__, "binaries", "run_token_counter.sh")
        count = subprocess.check_output(
            [token_counter_sh, seed_file_path], 
            stderr=subprocess.STDOUT)
        return int(count)
    except Exception as err:
        print("Error counting token for " + seed_file_path)
        raise err
...
def report_generator(subject, reducer, reducer_flags, token_count_seed, token_count_variant, iteration, run_result: list):
    """Report initialization"""
    report = dict()
    report['Subject'] = subject
    report['Reducer'] = reducer
    report["Environment"] = reducer_flags
    report["Origin seed token count"] = token_count_seed
    report["Origin variant token count"] = token_count_variant
    report["Iteration"] = iteration
    report["Query"] = int(run_result[3])
    report["Time"] = int(run_result[4])
    report["Token_remaining_seed"] = int(run_result[5])
    report["Token_remaining_variant"] = int(run_result[6])
    report["Ret_code"] = int(run_result[7])
    return report
...
def main():
    ...
    for subject_name in parameter.benchmark_target:
        ...
        # extract subject information (seed file, variant file and test script)
        seed_file_path, variant_file_path, script_file_path = extract_info_properties(subject_name)

        # count subject tokens
        token_count_seed = count_token(seed_file_path)
        print(f"Seed of subject {subject_name} has {token_count_seed} original tokens")
        token_count_variant = count_token(variant_file_path)
        print(f"Variant of subject {subject_name} has {token_count_variant} original tokens")
        ...
        for reducer in parameter.reducers:
            ...
            for iteration in range(parameter.iterations):
                ...
                with open(fname, "r") as output:
                    ...
                    if len(run_result) != 8:
                        run_result = ["unknown", "unknown", "unknown", "failed", "failed", "failed", "failed", run_result]
                os.remove(fname)

                report = report_generator(subject_name, reducer, reducer_flags, token_count_seed, token_count_variant, iteration, run_result)
