...
data class NumOfDeletesToPatterns(
  private val patterns: ImmutableList<ImmutableList<ElementEditPattern>>,
) {
  init {
    ...
    patterns.forEachIndexed { numOfDeletes, patterns ->
      ...
      check(patterns.distinct().count() == patterns.size)
    }
    patterns.asSequence()
  ...
  val patternsInUseDescending = ImmutableList.builder<ElementEditPattern>().apply {
    for (numOfDeletes in patterns.size - 1 downTo 2) {
      addAll(getPatternsWithGivenNumOfDeletes(numOfDeletes))
    }
  }.build()

  fun getPatternsWithGivenNumOfDeletes(numOfDeletes: Int): ImmutableList<ElementEditPattern> {
  ...
  companion object {
    ...
    fun createPatternsWithGivenNumOfDeletes(
